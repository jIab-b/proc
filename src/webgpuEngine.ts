...
import { customBlocks as customBlocksStore, nextTextureLayer, gpuHooks } from './stores'
...
export async function initWebGPUEngine(options: WebGPUEngineOptions) {
  ...
  function applyFaceBitmapsToGPU(bitmaps: Record<BlockFaceKey, ImageBitmap>, customBlock: CustomBlock) {
    ...
    const currentCustomLayers = Math.min(currentCustomBlocks.length, MAX_CUSTOM_BLOCKS) * blockFaceOrder.length
    ...
    if (customBlock.textureLayer === undefined) {
      const currentLayer = get(nextTextureLayer)
      if (currentLayer >= MAX_CUSTOM_BLOCKS) {
        console.warn(`Maximum custom blocks (${MAX_CUSTOM_BLOCKS}) reached. Cannot add more custom textures.`)
        return
      }
      customBlock.textureLayer = currentLayer
      nextTextureLayer.update(n => n + 1)
    }
