...
exports.generateRollingHills = generateRollingHills;
exports.createSimpleWorldConfig = createSimpleWorldConfig;
const chunks_1 = require("../chunks");
const noise_1 = require("./noise");
function generateRollingHills(chunk, config) {
    ...
    for (let x = 0; x < sx; x++) {
        for (let z = 0; z < sz; z++) {
            ...
            const elevation = (0, noise_1.fbmNoise2D)(x * baseFreq, z * baseFreq, seed, 4, 2.0, 0.5);
            ...
            for (let y = 0; y < sy; y++) {
                if (y > columnHeight) {
                    chunk.setBlock(x, y, z, chunks_1.BlockType.Air);
                }
                else if (y === columnHeight) {
                    // Top layer is grass
                    chunk.setBlock(x, y, z, chunks_1.BlockType.Grass);
                }
                else if (y >= columnHeight - 3) {
                    // 3 layers of dirt below grass
                    chunk.setBlock(x, y, z, chunks_1.BlockType.Dirt);
                }
                else {
                    // Everything else is stone
                    chunk.setBlock(x, y, z, chunks_1.BlockType.Stone);
                }
            }
        }
    }
}
function createSimpleWorldConfig(seed = Date.now()) {
    return {
        seed,
